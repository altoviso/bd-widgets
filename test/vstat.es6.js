...
smoke.defTest({
	...
	test: function(){
		...
		// Providing just an error level returns a vstat at that level and the default message for that level. The className accessor reflects the level. The is* accessors provide for expressive clarity.
		...
		let msgs = [[], [], [], [], [], [], []];
		let hasMethodName = "Valid.ContextInfo.ScalarInfo.ContextWarn.ScalarWarn.ContextError.ScalarError".split(".").map(name=>`has${name}Messages`);
		function assertMessages(){
			for(let level=VStat.VALID; level<=VStat.SCALAR_ERROR; level++){
				let actual = vs.getMessagesRaw(level);
				let expected = msgs[level];
		...
		// Individual messages can be deleted; deleting all messages at the highest level moves the overall level down to the highest level that still has messages.
