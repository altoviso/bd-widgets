...
function assertClassNameEq(_lhs, _rhs){
	let lhs = _lhs.trim().split(" ").sort();
	let rhs = _rhs.trim().split(" ").sort();
...
class TestContainer extends Component {
	constructor(kwargs){
		super(Object.assign({id: "test-container"}, kwargs));
		this._message = "";
		this._prompt = "";
		this.log = [];
		this._throttle = 0;
	...
	waitForValue(component, prop, value, actions){
		...
		return new Promise(resolve => {
			let h = component.watch(prop, (newValue) => {
	...
	bdElements(){
		let style = {margin: "1em", border: "1px solid black", padding: "1em"};
		return e("div", {className: "top"},
			e("div", {className: "widget-container", bdChildrenAttachPoint: true, style: style}),
